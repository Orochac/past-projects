...
main(int argc, char *argv[]){
    ...
    /* Get data from standard input then format and print the data
    */
    data = get_data();
    fill_grid(data, data->blocks->head);
    print_data(data);
    /* Print the grid before and after repair
    */
    printf("%s\n", STAGE1);
...
add_node(data_t *data, char *line, int type){
    ...
    if(type == BLOCK){
       ...
       data->route = insert_route(data->route, line);
        ...
        if(type-BLOCKSLIST ==data->blockslistlen){
            data->blockslistlen++;
            data->blockslist = (list_t**)realloc(data->blockslist,\
            data->blockslistlen*sizeof(list_t*));
            data->blockslist[type-BLOCKSLIST] = make_empty_list();
...
/**************************************************************************/
/* main program controls all the action 
 * From the lecture 7 slides modified with points ontdata->
 */
list_t
...
/**************************************************************************/
/* Takes the main data struct and prints as well as stores
...
/**************************************************************************/
/* Checks the validity of a move by testing if it is within + or - 1 of the
 * last move and not diagonal. returns 1 if valid 0 if not.
 */
int valid_move(point_t curpoint, point_t lastpoint){
...
/**************************************************************************/
/* Fill the grid with ''s and add '#'s where there is a block.
 */
void
...
/**************************************************************************/
/* Repair the grid of any blockages
*/
void
...
/**************************************************************************/
/* Traverse the queue and repair the broken route
*/
void
...
repair_route(list_t *rqueue, node_t *broute, data_t *data, node_t *repair){
    ...
    node_t *temp = data->route->head, *temps = fragment->head;
    
    while(temps != NULL){
        printf("[%d, %d]", temps->point.row, temps->point.col);
        temps = temps->next;
    }
    temps = broute;
    while(temp != NULL){
