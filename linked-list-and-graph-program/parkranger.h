...
// The directed acyclic graph is implemented using a tree,
// The tree contains: A deque of child verticies, a counter of parents
// and the node number 
typedef struct tree Tree;

// Create a new Tree with the given verticie number and return a pointer to it
Tree *new_tree(int data);

// Identify using a form of topological sorting if there is an unbroken path
// with max length of the total amount of verticies. 
bool max_path_len(Tree **list, int vertCount);

// read the edge inputs from stdin and return an array of trees with 
// appropriate edges.
Tree **read_input();

// Free memory associated with all trees contained in the array
void free_tree(Tree **tree, int vertCount);
